...
| **Reconnaissance CA — vente à crédit** (module Crédits) | **Accrual** (total le jour de la vente) | La marchandise est sortie : la vente compte en plein dans le CA le jour J, comme une vente cash → le dashboard ventes (« CA = valeur vendue du jour ») reste **inchangé**. L'impayé est une créance suivie dans le **dashboard crédits** (trésorerie), pas un manque de CA. Alternative caisse (n'encaisser que l'argent reçu) rejetée car elle aurait modifié le CA prouvé en Phase 5. |
| **Suppression d'un crédit** (module Crédits) | **Annulation traçable** (restock + reversal) | Un crédit = une vraie vente (stock sorti, paiements). Pas de hard-delete : l'annulation réintègre le stock et **reverse la vente via le mécanisme de retour existant** (le CA net se corrige tout seul, zéro changement aux requêtes prouvées) ; le crédit est marqué `cancelled` et exclu de la trésorerie. Ni le stock ni le CA ne sont faussés. |
| **Reste / statut crédit** (module Crédits) | **Dérivés** (jamais stockés) | `reste = total − Σ paiements` ; statut dérivé (Payé/Partiel/Non payé/Annulé). `paid` agrégé en SQL (pas de N+1, pas de colonne qui diverge). Total = somme des lignes (prix capturés), jamais saisi. |
| **Édition d'un crédit** (module Crédits) | Lignes/total **immuables** | Une fois créé, le stock est sorti et les prix capturés : on ne modifie pas les produits/total (sinon le reste devient incohérent). Seuls client et échéance restent éditables. Corriger une erreur = annuler (reversal) puis recréer. |
| **Dettes fournisseurs** (module Dettes) | **Aucun couplage stock ni CA** | Registre purement financier (comptes à payer) : pas de réception de marchandise, pas de chiffre d'affaires. Le lien produit est **descriptif** (FK nullable, jamais de mouvement de stock) ; le dashboard ventes/CA est intact. Un vrai flux d'achat (commande→réception→stock-in) serait un **module séparé**. |
| **Total d'une dette** (module Dettes) | **Saisi** (facture fournisseur) | Contrairement au crédit (total dérivé des produits vendus), le total vient de la facture externe → saisi par l'admin. Reste/statut restent **dérivés** (`reste = total − Σ paiements`, BigDecimal, agrégé SQL). |
| **Suppression d'une dette** (module Dettes) | **Bloquée si paiements** (sinon 204) | Une dette avec paiements = un historique de sorties d'argent réelles : suppression refusée (409) pour ne pas perdre l'historique. Total non modifiable une fois un paiement enregistré. Sans paiement, suppression simple autorisée. |
