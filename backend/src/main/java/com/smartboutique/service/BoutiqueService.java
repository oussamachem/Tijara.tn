...
public class BoutiqueService {
    ...
    private void seedDefaultCatalog(Long boutiqueId) {
        ...
        entityManager.createNativeQuery("SELECT set_config('app.current_boutique', :bid, true)")
                ...
                .getSingleResult();
        // Idempotent : on n'insere une couleur/taille par defaut que si elle n'existe pas deja dans
        // le scope courant. En prod (unicite PAR BOUTIQUE) une nouvelle boutique part vide -> tout est
        // seede ; si l'appel est rejoue, aucun doublon. Robuste aussi au schema de test create-drop
        // (unicite GLOBALE sur le nom) : la 2e boutique ne re-tente pas d'inserer un nom deja pris.
        for (DefaultColor c : DEFAULT_COLORS) {
            if (!colorRepository.existsByName(c.name())) {
